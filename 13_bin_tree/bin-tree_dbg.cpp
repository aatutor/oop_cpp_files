...
std::ostream& operator<<(std::ostream& out, const ElemTree& obj) {
	obj >> out;
	return out;
}
...
/// TODO: fix last ','
void ElemTree::operator>> (std::ostream& out) const {
	Print(root_, out);
}
...
void ElemTree::Print(Node* obj, std::ostream& out) const {
	if (obj == nullptr)
		return;
...
Node* ElemTree::Next(Node* obj) const {
	...
	while (obj == curParent->right && curParent != nullptr) {
		obj = curParent;
...
Node* ElemTree::Search(int key, Node* temp) const {
	while(temp){
